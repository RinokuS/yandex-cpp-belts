...
string GetCommentByCode(HttpCode iCode)
{
    switch (iCode) {
        case HttpCode::Ok:
            return "OK";
...
public:
    ...
    {
        output << "HTTP/1.1 " << static_cast<std::underlying_type<HttpCode>::type>(resp._code) << ' ' <<
               GetCommentByCode(resp._code) << '\n';

        bool clHedaerAdded = false;
        for (auto h : resp._headers) {
            if (h.name != "Content-Length")
                output << h.name << ": " << h.value << '\n';
            else if (!resp._content.empty() && !clHedaerAdded) {
                output << "Content-Length" << ": " << resp._content.size() << '\n';
                clHedaerAdded = true;
            }
        }
        if(!resp._content.empty() && !clHedaerAdded)
            output << "Content-Length" << ": " << resp._content.size() << '\n';
        output << '\n' << resp._content;
        return output;
    }
    ...
    HttpResponse ServeRequest(const HttpRequest& req)
    {
        ...
        if (req.method == "POST") {
            if (req.path == "/add_user") {
                ...
                result.SetCode(HttpCode::Ok).AddHeader("Content-Length", to_string(response.size())).SetContent(response);
            }
            ...
            if (req.path == "/user_comments") {
                ...
                result.SetCode(HttpCode::Ok).AddHeader("Content-Length", to_string(80)).SetContent(
