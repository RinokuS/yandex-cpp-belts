...
public:
    ...
    struct WriteAccess {
        ...
        WriteAccess(const K& key, mutex& m, MapType& bucket_content)
                : guard(m)
                , ref_to_value(bucket_content[key])
        {
        }
    };
    ...
    struct ReadAccess {
        ...
        ReadAccess(mutex& m, const V& val)
                : guard(m)
                , ref_to_value(val)
        {
        }

        ReadAccess(const K& key, mutex& m, const MapType& bucket_content)
                : guard(m)
                , ref_to_value(bucket_content.at(key))
        {
        }
    };

    explicit ConcurrentMap(size_t bucket_count) : _maps(vector<MapType>(bucket_count)),
        _mutexes(vector<mutex>(bucket_count)) {}

    WriteAccess operator[](const K& key)
    {
        size_t mapIdx = hasher(key) % _maps.size();

        return { key, _mutexes[mapIdx], _maps[mapIdx] };
    }
    ...
    bool Has(const K& key) const{
        size_t mapIdx = hasher(key) % _maps.size();
        const auto& bucket = _maps[mapIdx];
        Lock(_mutexes[mapIdx]);
    ...
    {
        MapType resultMap;
        for (size_t mapIdx = 0; mapIdx < _maps.size(); ++mapIdx) {
            lock_guard<mutex> g(_mutexes[mapIdx]);
            resultMap.insert(_maps[mapIdx].begin(), _maps[mapIdx].end());
        }
        return resultMap;
    }
