...
public:
    ...
    PersonalBudget& PayTax(const Date& from, const Date& to, int percentage);
...
private:
    ...
    vector<double> spent=vector<double>(days_in_XXI_century, 0.0);
...
PersonalBudget& PersonalBudget::Earn(const Date& date_from, const Date& date_to, double value){
    ...
    unsigned int start_day = ComputeDaysDiff(date_from, begin_XXI_century),
            end_day = start_day+days_count;
    const auto amountPerDay = value/days_count;
    for (auto i = start_day; i < end_day; ++i)
        earn[i] += amountPerDay;
...
PersonalBudget& PersonalBudget::Spend(const Date& date_from, const Date& date_to, double value){
    ...
    const auto amountPerDay = value/days_count;
    for (auto i = start_day; i < end_day; ++i)
        spent[i] += amountPerDay;

    return *this;
}

PersonalBudget& PersonalBudget::PayTax(const Date& date_from, const Date& date_to, int percentage){
    const double afterTax = (100.0 - percentage) / 100.0;
    unsigned int start_day = ComputeDaysDiff(date_from, begin_XXI_century),
...
double PersonalBudget::ComputeIncome(const Date& date_from, const Date& date_to){
    int start_day = ComputeDaysDiff(date_from, begin_XXI_century),
            end_day = ComputeDaysDiff(date_to, begin_XXI_century);
    double all_budget = accumulate(earn.begin()+start_day,
                                   earn.begin()+end_day+1, 0.0);
    double spent_budget = accumulate(spent.begin()+start_day,
                                     spent.begin()+end_day+1, 0.0);
    return all_budget - spent_budget;
}
